...
calculator = MetricsCalculator(
    dataset=Datasets.TELCO, labels=labels, predictions=predictions, scores=scores
)
...
# %%
def plot_scores_and_predictions(
    scores, labels, predictions, data, output, start_index=0, end_index=-1
):
    """
...
# %%
def plot_score_histogram(scores, labels, threshold, metrics=None, range_based=False):
    # if metrics is not None:
    # pprint(metrics)

    scores = scores.numpy().flatten()
    labels = labels.numpy().flatten()
    # Aggregate scores and labels across all series
    ...
    if range_based:
        ...
        anomalous_scores = np.array(max_anomalous_scores)
...
plot_score_histogram(scores, labels, threshold[0], metrics, range_based=True)
plot_score_histogram(scores, labels, threshold[0], metrics, range_based=False)

# %%


def plot_score_histogram_with_ranges(
    scores, labels, threshold, metrics=None, range_size=10
):
    if metrics is not None:
        pprint(metrics)
...
plot_score_histogram_with_ranges(
    scores[:, 11], labels[:, 11], threshold[11], metrics_per_class[11]
)

# %%
for i in range(scores.shape[1]):
    plot_score_histogram_with_ranges(
        scores[:, i], labels[:, i], threshold[i], metrics_per_class[i]
    )
    plot_scores_and_predictions(
        scores[:, i],
        labels[:, i],
        predictions[:, i],
        data[:, i],
...
# %%
def plot_score_histograms_grid(
    scores, labels, thresholds, metrics_per_class, model_name="GRU"
):
...
plot_score_histograms_grid(
    scores, labels, threshold, metrics_per_class, model_name="GRU"
)


# %%
def plot_score_histograms_grid_with_ranges(
    scores, labels, thresholds, metrics_per_class, model_name="GRU"
):
    num_classes = scores.shape[1]
    num_rows = 4
...
def print_telco_metrics(base_path, model_name, save_path=None):
    for split in ["test", "train", "val"]:
        data_path = os.path.join(
            base_path, f"{split}_{model_name.lower()}_v6_telco_data.pt"
        )
        labels_path = os.path.join(
            base_path, f"{split}_{model_name.lower()}_v6_telco_labels.pt"
        )
        output_path = os.path.join(
            base_path, f"{split}_{model_name.lower()}_v6_telco_output.pt"
        )
        predictions_path = os.path.join(
            base_path, f"{split}_{model_name.lower()}_v6_telco_predictions.pt"
        )
        scores_path = os.path.join(
            base_path, f"{split}_{model_name.lower()}_v6_telco_scores.pt"
        )
        ...
        for metric_name, metric_value in metrics.items():
            if "per_class" in metric_name:
                for i in range(scores.shape[1]):
                    metrics_per_class[i][metric_name.split("_per_class")[0]] = (
                        metric_value[i]
                    )
        fig_grid = plot_score_histograms_grid(
            scores, labels, threshold, metrics_per_class, model_name=model_name
        )
        fig_grid_with_ranges = plot_score_histograms_grid_with_ranges(
            scores, labels, threshold, metrics_per_class, model_name=model_name
        )
        if save_path:
            ...
            fig_grid.savefig(
                os.path.join(
                    save_path,
                    f"{split}_{model_name.lower()}_v6_telco_score_histogram.png",
                )
            )
            fig_grid_with_ranges.savefig(
                os.path.join(
                    save_path,
                    f"{split}_{model_name.lower()}_v6_telco_score_histogram_with_ranges.png",
                )
            )
...
threshold_flattened = threshold.flatten()
metrics_flattened = metrics
