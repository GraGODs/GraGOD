...
from models.gdn_god.modules import GNNLayer, OutLayer


class GDNGOD(nn.Module):
    """
    ...
    def forward(self, data: torch.Tensor) -> torch.Tensor:
        ...
        for i, edge_index in enumerate(edge_index_sets):
            ...
            gated_edge_index = self._get_gated_edge_index(
                cos_ji_mat, self.topk, node_num
            )
            ...
            indexes = topk_indices_ji.view(node_num * self.topk, 1).repeat(batch_num, 1)
    ...
    def _get_gated_edge_index(self, cos_ji_mat, topk, node_num):
        """Get the gated edge index based on cosine similarity matrix.

        Takes a cosine similarity matrix and returns the edge indices for the top-k
        most similar node pairs in COO format. The similarity matrix is flattened and
        global top-k values are selected to identify the most important edges.

        Args:
            cos_ji_mat: Cosine similarity matrix between nodes j and i
            topk: Number of top edges to select per node
            node_num: Number of nodes in the graph
            device: Device to place tensors on

        Returns:
            torch.Tensor: Edge index tensor in COO format of shape [2, total_edges]
                where edge_index[0] contains target node indices (j) and
                edge_index[1] contains source node indices (i)
        """
        flat_cos = cos_ji_mat.flatten()
        total_edges = topk * node_num

        topk_values, topk_indices = torch.topk(flat_cos, total_edges)

        row_indices = topk_indices // cos_ji_mat.size(1)
        col_indices = topk_indices % cos_ji_mat.size(1)

        edge_index = torch.stack([col_indices, row_indices], dim=0)

        return edge_index


class GDNGOD_PLModule(PLBaseModule):
    """
