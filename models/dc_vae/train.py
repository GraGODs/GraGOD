...
def main(
    ...
    train_loader = DataLoader(
        ...
        num_workers=0,  # Set to 0 when data is on GPU
        shuffle=shuffle,
        pin_memory=False,  # Set to False since data is already on GPU
    )
    ...
    trainer = TrainerPL(
        model=model,
        model_pl=DCVAE_PLModule,
        model_params=params["model_params"],
        criterion={},  # VAE handles its own loss computation
        batch_size=batch_size,
        n_epochs=n_epochs,
        init_lr=init_lr,
        device=device,
